...
def save_data(d):
    # Atomic write: protects the saved session/data from partial JSON writes.
...
def persist_client_settings(acc_id, cl):
    """Persist the complete instagrapi client state server-side."""
    try:
        ...
        with data_lock:
            d = load_data()
            if acc_id not in d.get("accounts", {}):
                return False
            d["accounts"][acc_id]["session_settings"] = settings
            d["accounts"][acc_id]["session_saved_at"] = time.time()
            d["accounts"][acc_id]["session_status"] = "saved"
            save_data(d)
        return True
    except Exception:
        return False

def get_client(acc_id, session_id, proxy=None, csrf_token=None, force_session_login=False):
    ...
    if saved_settings and not force_session_login:
        try:
            cl = Client()
    ...
    if not session_id:
        raise RuntimeError("Session ID is empty")
    cl.login_by_sessionid(session_id)
    # Verify the session is actually usable, then save it BEFORE any message action.
    cl.account_info()
    if not persist_client_settings(acc_id, cl):
        raise RuntimeError("Could not save authenticated session state")
    ig_clients[acc_id] = cl
    return cl
            ...
            try:
                ...
                if status_code == 403 or "user_has_logged_out" in err_str or "login_required" in err_str:
                    ...
                    try:
                        ...
                        # Freshly authenticate from the stored sessionid.
                        # get_client validates it and saves the new client state first.
                        cl = get_client(acc_id, session_id, proxy, csrf_token, force_session_login=True)
                        bot_status[acc_id]["reauth_attempted"] = False
                        log(acc_id, "💾 Fresh session authenticated + saved")
                        log(acc_id, "✅ Re-auth successful — resuming")
                        bot_status[acc_id]["last_action"] = "Session saved ✓"
...
<div class="modal">
  ...
  <div class="form-section">
    ...
    <div class="form-grid">
      ...
      <div class="form-group"><label>SESSIONID</label><input type="text" id="f-sid" placeholder="sessionid" autocomplete="off"/><div class="hint">Session state is saved server-side after successful authentication.</div></div>
  ...
  ids.forEach(id => {
    ...
    existing.innerHTML = `
      <div class="acc-header">
        ...
        <div class="session-badge" style="color:${acc.session_saved ? 'var(--green)' : 'var(--red)'}">${acc.session_saved ? '● SESSION SAVED' : '○ SESSION NOT SAVED'}</div>
...
def panel_page():
    ...
    if login_role == "user":
        with data_lock:
            ...
            # Backfill the permanent uptime anchor for users created before
            # the timer field existed. It is written once and then reused.
            try:
                user_created_at_epoch = float(user.get("created_at_epoch", 0) or 0)
            except (TypeError, ValueError):
                user_created_at_epoch = 0
            if user_created_at_epoch <= 0 and user:
                user_created_at_epoch = time.time()
                user["created_at_epoch"] = user_created_at_epoch
                users[login_username] = user
                save_data(d)
...
def get_accounts():
    ...
    for acc_id, acc in visible_accounts(d).items():
        ...
        result[acc_id] = {
            ...
            "session_saved":  bool(acc.get("session_settings")),
            "session_saved_at": acc.get("session_saved_at", 0),
            "session_status": acc.get("session_status", "not_saved"),
            "proxy":          acc.get("proxy", ""),
...
def add_account():
    ...
    try:
        ...
        if temp_cl:
            ...
            entry["session_saved_at"] = time.time()
            entry["session_status"] = "saved"
    except Exception:
        pass
    with data_lock:
        d = load_data()
